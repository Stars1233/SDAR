"""Entry point for fixed-coefficient K3-IS OPD training."""
...
def main(config):
    run_opd_trainer(config)


def configure_opd_trainer(config) -> dict:
    """Configure the actor and frozen teacher for pure OPD training."""
...
def run_opd_trainer(config) -> None:
...
class OPDTaskRunner:
    def run(self, config):
        ...
        settings = configure_opd_trainer(config)
        ...
        if config.actor_rollout_ref.actor.strategy not in ("fsdp", "fsdp2"):
            raise NotImplementedError("OPDTrainer currently supports FSDP/FSDP2 actors")
        ...
        if reward_manager_name != "episode":
            raise NotImplementedError(f"Unsupported validation reward manager: {reward_manager_name}")
        from agent_system.reward_manager import EpisodeRewardManager

        val_reward_fn = EpisodeRewardManager(tokenizer=tokenizer, num_examine=1, normalize_by_length=False)

        assert config.actor_rollout_ref.rollout.n == 1, "actor_rollout_ref.rollout.n must stay 1; environment grouping uses env.rollout.n"
        ...
        print(f"[OPD] teacher_model_path: {settings['teacher_model_path']}")
        print(f"[OPD] opd_coef: {settings['opd_coef']}")
        print(f"[OPD] skills_dir: {skills_dir}")

        from verl.trainer.ppo.opd_trainer import OPDTrainer
